...
const  config={
  ...
  spritesmith:{
    dirname : 'item/images/icon-*.png',
    dist    : 'item'
  },
  img:{
    dirname : ['item/images/*.*','!item/images/icon-*.png'],
    dist    : 'dist/images'
  },
  cssLint:{
    dirname : 'item/css/*.css'
  },
  cssHack:{
    dirname : 'dist/css/*.css',
    dist    : 'item/css'
  },
  css:{
    dirname : 'item/css/*.css',
...
gulp.task('css', function() {
  ...
  return gulp.src(config.css.dirname)
    ...
    //.pipe(postcss(processors))//验证css是否符合条件
...
gulp.task('default', function(){
  gulp.start('spritesmith','html','img', 'css', 'js','js-lint','webserver');
 
  //监听html文件变化
  gulp.watch(dirname + '**/*.html', function(){
    gulp.run('html');
  });
 
  // Watch .css files
  gulp.watch(dirname +'/css/*.css', function(){
    gulp.run('css');
  });

  // Watch .js files
  gulp.watch(dirname +'/js/*.js', function(){
    gulp.run('js');
  });
 
  // Watch image files
  gulp.watch([dirname + '/images/*.*','!'+dirname + '/images/icon-*.png'], function(){
    gulp.run('img');
  });

  gulp.watch(dirname + '/images/icon-*.png', function(){
